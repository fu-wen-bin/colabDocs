      ...
      <main className="container mx-auto px-4 py-6 "
            ...
            {docStatus.map((item, idx) => (
              ...
              >
                ...
                <div className="flex-grow">
                  <p
                    className="text-gray-500 text-sm font-medium">{item.name}</p>
                  <h3
                    className="text-2xl font-bold text-gray-800 mt-0.5">{item.value}</h3>
                  {item.desc &&
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <div className="mb-4 pb-3 border-b border-gray-100">
            ...
            <p
              className="text-gray-500 text-sm mt-1">最近更新或创建的文档列表</p>
          </div>
            ...
            {recentDocs.map((doc) => (
              ...
              >
                ...
                <div className="flex items-center">
                  <div className="mr-3 p-2 bg-gray-100 rounded">{getDocIcon(
                    doc.type)}</div>
                  <div>
                    <div className="flex items-center">
                      <span
                        className="text-gray-800 font-medium">{doc.title}</span>
                      {doc.isNew && (
                        <span
                          className="ml-2 text-xs bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded-full">
                        新
                      </span>
                      )}
                    </div>
